...
export const metadata = {
    title: "Education Platform - Triumph Synergy Testnet",
    description: "Courses, books, meal plans, and clothing - all paid with Pi/TriSyn",
};

export default async function EducationPage() {
    const session = await auth();
    if (!session) redirect("/api/auth/guest?redirectUrl=/testnet-hub/education");
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-400">Courses, books, meal plans, and student clothing</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div className="mb-8">
                    <div className="flex gap-4">
                        <input
                            ...
                            className="flex-1 rounded-lg bg-black/40 border border-green-500/30 px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-green-400 transition-colors"
                        />
                        <select className="rounded-lg bg-black/40 border border-green-500/30 px-4 py-3 text-white focus:outline-none focus:border-green-400 transition-colors">
                ...
                <div className="mb-16">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {courses.map((course) => (
                            <div key={course.id} className="rounded-xl bg-gradient-to-br from-green-900/40 to-emerald-900/40 border border-green-500/20 hover:border-green-400/60 overflow-hidden transition-all hover:shadow-lg hover:shadow-green-500/20">
                                <div className="p-6 bg-black/30">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mb-4 text-center text-xs">
                                        ...
                                        <div className="rounded bg-black/50 p-2">
                                            <p className="text-gray-400">📚</p>
                                            <p className="font-semibold text-white">{course.modules} modules</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="px-6 py-3 border-t border-green-500/10 bg-gradient-to-r from-green-900/20 to-emerald-900/20">
                                    <div className="flex justify-between items-center mb-3">
                                        <span className="font-bold text-green-400">{course.price}</span>
                                    </div>
                                    <button className="w-full py-2 rounded-lg bg-gradient-to-r from-green-500/50 to-emerald-500/50 hover:from-green-500 hover:to-emerald-500 font-semibold text-white transition-all">
                                        Enroll Now
                                    </button>
                                </div>
                            </div>
                        ...
                        {books.map((book) => (
                            <div key={book.id} className="rounded-xl bg-gradient-to-br from-blue-900/40 to-indigo-900/40 border border-blue-500/20 hover:border-blue-400/60 p-6 transition-all hover:shadow-lg hover:shadow-blue-500/20">
                                ...
                                <div className="flex gap-3">
                                    <span className="flex-1 py-2 rounded-lg bg-black/40 font-bold text-center text-blue-400">{book.price}</span>
                                    <button className="flex-1 py-2 rounded-lg bg-gradient-to-r from-blue-500/50 to-indigo-500/50 hover:from-blue-500 hover:to-indigo-500 font-semibold text-white transition-all">
                                        Buy Now
                                    </button>
                                </div>
                            </div>
                        ...
                        {mealPlans.map((plan) => (
                            <div key={plan.id} className="rounded-xl bg-gradient-to-br from-purple-900/40 to-pink-900/40 border border-purple-500/20 hover:border-purple-400/60 overflow-hidden transition-all hover:shadow-lg hover:shadow-purple-500/20">
                                ...
                                <div className="px-6 py-3 border-t border-purple-500/10 bg-gradient-to-r from-purple-900/20 to-pink-900/20">
                                    <div className="flex justify-between items-center mb-3">
                                        <span className="font-bold text-purple-400 text-lg">{plan.price}</span>
                                    </div>
