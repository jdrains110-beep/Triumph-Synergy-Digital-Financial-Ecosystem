...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!verifyResponse.ok) {
      console.error('[Pi Payment API] Payment verification failed:', verifyResponse.status);
    ...
    if (isAdmin && metadata?.adminOverride) {
      console.log('[Pi Payment API] Admin override applied for completion:', paymentId);
    }
    ...
    if (!completeResponse.ok) {
      console.error('[Pi Payment API] Payment completion failed:', completeResponse.status);
      return NextResponse.json({
    ...
    try {
      ...
      console.warn('[Pi Payment API] Network verification failed:', networkError);
    }
