	...
	try {
		switch (action) {
			...
			case "notifications": {
				...
				const notifications =
					await piosIntegration.getUserNotifications(userId);
			...
			case "rates": {
				const dualRates = piosIntegration.getDualRateInfo();
				return NextResponse.json({
					success: true,
					rates: {
						internal: {
...
export async function POST(request: NextRequest) {
	try {
		...
		switch (action) {
			...
			case "cancel-payment": {
				const cancelled = await piosIntegration.cancelPayment(
					body.paymentId,
					body.reason,
				);
			...
			case "update-triumph-profile": {
				const updated = await piosIntegration.updateUserTriumphProfile(
					body.userId,
					{
