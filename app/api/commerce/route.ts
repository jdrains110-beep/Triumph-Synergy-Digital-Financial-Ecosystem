...
import { ecommerceHub, type VendorCategory, type ProductCategory } from "@/lib/commerce/ecommerce-hub";
  ...
  try {
    ...
    switch (action) {
      ...
      case "products": {
        ...
        const categoryParam = searchParams.get("category");
        const category = categoryParam ? categoryParam as ProductCategory : undefined;
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case "create-product": {
        const product = await ecommerceHub.createProduct({ vendorId: data.vendorId, ...data.product });
        return NextResponse.json({
          success: true,
          data: product,
      ...
      case "add-to-cart": {
        const cart = await ecommerceHub.addToCart(data.userId, data.productId, data.quantity, data.variantId);
        return NextResponse.json({
          success: true,
          data: cart,
      ...
      case "update-cart": {
        const cart = await ecommerceHub.updateCartItem(data.userId, data.itemId, data.quantity);
        return NextResponse.json({
          success: true,
          data: cart,
      ...
      case "apply-coupon": {
        const cart = await ecommerceHub.applyCoupon(data.userId, data.couponCode);
        return NextResponse.json({
          success: true,
          data: cart,
