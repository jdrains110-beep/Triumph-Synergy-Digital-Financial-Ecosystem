...
CREATE TABLE IF NOT EXISTS saib_security_logs (
  ...
  event_type TEXT NOT NULL CHECK (event_type IN ('OMNIGUARD_AUDIT', 'GAS_MARKET_CHECK', 'CIRCUIT_BREAKER', 'STANDARD_EXECUTION')),
...
-- ============================================================
-- TABLE 3: Reentrancy Audit History
-- ============================================================
...
CREATE POLICY "allow_service_role_all" ON saib_security_logs
  USING (true)
  WITH CHECK (true);

CREATE POLICY "allow_service_role_all" ON ecosystem_balances
  USING (true)
  WITH CHECK (true);

CREATE POLICY "allow_service_role_all" ON omniguard_audits
  USING (true)
  WITH CHECK (true);

CREATE POLICY "allow_service_role_all" ON gas_price_history
  USING (true)
  WITH CHECK (true);

CREATE POLICY "allow_service_role_all" ON system_events
  USING (true)
  WITH CHECK (true);
...
SELECT 
  ...
  COUNT(CASE WHEN circuit_breaker_tripped THEN 1 END) as circuit_breaker_trips,
