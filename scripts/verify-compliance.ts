...
interface ComplianceReport {
  ...
  overallStatus: 'FULLY_COMPLIANT' | 'SUBSTANTIALLY_COMPLIANT' | 'NON_COMPLIANT';
...
function verifyCompliance(): ComplianceReport {
  const compliantCount = complianceChecklist.filter(req => req.status === 'COMPLIANT').length;
  const totalRequirements = complianceChecklist.length;
  const compliancePercentage = (compliantCount / totalRequirements) * 100;

  let overallStatus: 'FULLY_COMPLIANT' | 'SUBSTANTIALLY_COMPLIANT' | 'NON_COMPLIANT';
...
function printComplianceReport() {
  ...
  console.log(`Compliance Score: ${report.compliancePercentage.toFixed(1)}% (${report.compliantCount}/${report.totalRequirements})`);
  console.log('\n');
  ...
  const statusCounts = {
    COMPLIANT: complianceChecklist.filter(r => r.status === 'COMPLIANT').length,
  ...
  report.requirements.forEach((req, index) => {
    const statusSymbol = req.status === 'COMPLIANT' ? '✅' : req.status === 'PENDING' ? '⏳' : '❌';
    console.log(`\n${index + 1}. ${statusSymbol} ${req.requirement}`);
    console.log(`   Status: ${req.status}`);
    console.log(`   Last Verified: ${new Date(req.lastVerified).toLocaleDateString()}`);
    console.log(`   Evidence:`);
    req.evidence.forEach(ev => console.log(`     • ${ev}`));
    console.log(`   Next Review: ${new Date(req.nextReview).toLocaleDateString()}`);
  });
  ...
  console.log('─'.repeat(80));
  console.log(`Next Comprehensive Audit: ${new Date(report.nextAuditDate).toLocaleDateString()}`);
...
// EXPORT FOR TESTING
export { verifyCompliance, checkDeploymentReadiness, complianceChecklist, type ComplianceReport };
