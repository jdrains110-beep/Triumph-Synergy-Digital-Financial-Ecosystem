...
`,
  ...
  async connectPiDexSDK(): Promise<GitHubRepository> {
    // Connect to kosasih/pidexsdk repository for Pi DEX smart contract integration
    return this.connectGitHubRepository("kosasih", "pidexsdk");
  }

  async integratePiDexSmartContract(): Promise<SmartContract> {
    // Sync the main Pi DEX smart contract from kosasih/pidexsdk
    const repoFullName = "kosasih/pidexsdk";
    
    // First ensure repository is connected
    if (!this.repositories.has(repoFullName)) {
      await this.connectPiDexSDK();
    }

    // Sync the main DEX contract (assuming it exists at contracts/dex.rs or similar)
    const contract = await this.syncContractFromGitHub(
      repoFullName,
      "contracts/pi_dex.rs", // Adjust path based on actual repo structure
      "main"
    );
