...
export const contractSignatures = pgTable(
  ...
  (table) => ({
    ...
    contractUserUnique: uniqueIndex('idx_signatures_contract_user').on(table.contractId, table.userId),
  })
);
...
export const userConsents = pgTable(
  'user_consents',
  {
    ...
    consentStatus: varchar('consent_status', { length: 50 }).notNull().default('NOT_REQUESTED'),
  ...
  (table) => ({
    ...
    userContractUnique: uniqueIndex('idx_consents_user_contract').on(table.userId, table.contractId),
  })
);
