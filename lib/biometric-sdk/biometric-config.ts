...
export type AuthenticatorTransport =
  ...
  | "hybrid"
  | "platform";
export type AttestationConveyanceFormat =
  | "direct"
  | "indirect"
  | "none"
  | "enterprise";
...
export type BiometricRegistrationOptions = {
  userId: string;
  userName: string;
  userEmail: string;
  displayName: string;
...
export type FallbackAuthOptions = {
  method: "pin" | "password" | "otp" | "security_question";
  value: string;
  question?: string;
};

export type PlatformBiometricConfig = {
  platform: string;
...
export function getPlatformConfig(): PlatformBiometricConfig | null {
  ...
  return {
    ...
    supports: {
      ...
      conditionalUI:
        platform === "macos" || platform === "windows" || platform === "ios",
    },
  };
}
