...
export type LocationStatus = "planning" | "construction" | "active" | "maintenance" | "closed";

export type DeliveryCarrier = "usps" | "ups" | "fedex" | "dhl" | "trucking" | "local-courier" | "drone" | "autonomous";

export type DeliverySpeed = "same-day" | "next-day" | "2-day" | "standard" | "economy" | "freight";

export interface PhygitalLocation {
  id: string;
...
export interface DeliveryCarrierConfig {
...
export interface LocationSubscriptionPlan {
...
class PhygitalLocationsHub {
  private locations: Map<string, PhygitalLocation> = new Map();
  private deliveryOrders: Map<string, DeliveryOrder> = new Map();
  private truckingPartners: Map<string, TruckingPartner> = new Map();
  ...
  getLocationSubscriptionPlans(): LocationSubscriptionPlan[] {
    return [
      ...
      },
      {
        ...
        maxCarriers: 5,
        services: ["product-pickup", "product-dropoff", "returns", "customer-service", "pi-exchange", "pi-wallet-setup"],
      ...
      },
      {
        ...
        annualFee: 47988,
        annualFeeInPi: 47988 / PI_EXTERNAL_RATE,
        features: [
          "Up to 10,000 sq ft",
        ...
        maxCarriers: 10,
        services: ["product-pickup", "product-dropoff", "returns", "customer-service", "pi-exchange", "pi-wallet-setup", "product-demo", "repairs"],
      ...
      },
      {
        ...
        monthlyFee: 14999,
        monthlyFeeInPi: 14999 / PI_EXTERNAL_RATE,
        annualFee: 143988,
        annualFeeInPi: 143988 / PI_EXTERNAL_RATE,
        features: [
          "Up to 50,000 sq ft",
        ...
        maxSquareFootage: 50000,
        maxMonthlyOrders: 100000,
        maxCarriers: 15,
        services: ["product-pickup", "product-dropoff", "returns", "customer-service", "pi-exchange", "b2b-fulfillment", "cold-storage", "international-shipping"],
        supportLevel: "dedicated",
        platformFee: 2,
      ...
      },
      {
        ...
        maxCarriers: 999,
        services: ["product-pickup", "product-dropoff", "returns", "customer-service", "pi-exchange", "pi-wallet-setup", "product-demo", "repairs", "custom-orders", "b2b-fulfillment", "cold-storage", "hazmat-handling", "international-shipping"],
        supportLevel: "dedicated",
        platformFee: 1.5,
  ...
  async createLocation(data: {
    ...
    const location: PhygitalLocation = {
      ...
      is24Hours: data.type === "distribution-center" || data.type === "warehouse",
      ...
      hasBlockchainNode: data.subscriptionTier === "distribution" || data.subscriptionTier === "enterprise",
      internetSpeed: 1000,
      backupPower: true,
  ...
  async createDeliveryOrder(data: {
    ...
    const carrierConfig = location.deliveryCarriers.find((c) => c.carrier === data.carrier);
    ...
    const deliveryDays = data.speed === "same-day" ? 0 
      : data.speed === "next-day" ? 1 
      : data.speed === "2-day" ? 2 
      : data.speed === "standard" ? 5 
      : 7;
  ...
  async updateDeliveryStatus(orderId: string, status: DeliveryOrder["status"], location: string, description: string): Promise<DeliveryOrder> {
    ...
    if (status === "delivered") {
      ...
      if (loc) {
        loc.deliverySuccessRate = ((loc.deliverySuccessRate * (loc.ordersProcessedDaily - 1)) + 100) / loc.ordersProcessedDaily;
      }
    }
  ...
  async trackDelivery(trackingNumber: string): Promise<DeliveryOrder | null> {
    return Array.from(this.deliveryOrders.values()).find((o) => o.trackingNumber === trackingNumber) || null;
  }
  ...
  async registerTruckingPartner(data: {
    ...
    const partner: TruckingPartner = {
      ...
      monthlyFeeInPi: tierFees[data.subscriptionTier].monthly / PI_EXTERNAL_RATE,
      onTimeRate: 0,
      safetyRating: 0,
      totalDeliveries: 0,
      acceptsPi: true,
  ...
  getCarrierOptions(): { carrier: DeliveryCarrier; name: string; logo: string }[] {
    return [
      { carrier: "usps", name: "USPS - United States Postal Service", logo: "/carriers/usps.png" },
      { carrier: "ups", name: "UPS - United Parcel Service", logo: "/carriers/ups.png" },
      { carrier: "fedex", name: "FedEx", logo: "/carriers/fedex.png" },
      { carrier: "dhl", name: "DHL Express", logo: "/carriers/dhl.png" },
      { carrier: "trucking", name: "Freight & Trucking Partners", logo: "/carriers/trucking.png" },
      { carrier: "local-courier", name: "Local Courier Services", logo: "/carriers/local.png" },
      { carrier: "drone", name: "Drone Delivery (Coming Soon)", logo: "/carriers/drone.png" },
      { carrier: "autonomous", name: "Autonomous Vehicle Delivery", logo: "/carriers/autonomous.png" },
    ];
  }
