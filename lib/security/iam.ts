...
export const iamConfig = {
  ...
  authorization: {
    ...
    roles: {
      ...
      // Developer
      developer: {
        permissions: ["api:*", "webhooks:*", "logs:read", "docs:read"],
  ...
  apiAccess: {
    ...
    rateLimit: {
      ...
      tiers: {
        ...
        basic: { requests: 10_000, window: 3600 },
        pro: { requests: 100_000, window: 3600 },
        enterprise: { requests: 1_000_000, window: 3600 },
      },
...
export class IAMService {
  ...
  async hasPermission(userId: string, permission: string): Promise<boolean> {
    ...
    for (const role of roles) {
      const rolePerms =
        iamConfig.authorization.roles[
          role as keyof typeof iamConfig.authorization.roles
        ]?.permissions || [];
