 ...
 */
export function getSeverityForEvent(
  eventType: BiometricAuditEventType
...
export class BiometricAuditLogger {
  ...
  private static maxLogs = 10_000; // Keep last 10000 logs in memory
  ...
  static async log(
    ...
    // Store in memory
    BiometricAuditLogger.logs.push(log);
    if (BiometricAuditLogger.logs.length > BiometricAuditLogger.maxLogs) {
      BiometricAuditLogger.logs.shift();
    }

    // Send to backend for persistent storage
    try {
      await BiometricAuditLogger.persistLog(log);
    } catch (error) {
  ...
  static getLogs(filter?: {
    ...
    let filtered = [...BiometricAuditLogger.logs];
   ...
   * Clear old logs
   */
  static clearOldLogs(olderThanDays = 90): void {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - olderThanDays);

    BiometricAuditLogger.logs = BiometricAuditLogger.logs.filter(
  ...
  static getStats(userId?: string): {
    ...
    let logs = BiometricAuditLogger.logs;
   ...
   */
  static detectSuspiciousActivity(userId: string, windowMinutes = 5): boolean {
    const now = new Date();
    const windowStart = new Date(now.getTime() - windowMinutes * 60 * 1000);

    const recentLogs = BiometricAuditLogger.logs.filter(
      (log) =>
        log.userId === userId &&
