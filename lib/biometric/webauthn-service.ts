...
export class WebAuthnService {
  private rpName = 'Triumph Synergy';
  private rpId = typeof window !== 'undefined' 
    ? window.location.hostname 
    : process.env.NEXT_PUBLIC_APP_DOMAIN || 'localhost';
  private origin = typeof window !== 'undefined'
  ...
  async verifyAuthenticationResponse(
    ...
    if (newCounter <= storedCredential.counter) {
      throw new Error('Counter verification failed - possible credential clone');
    }
  ...
  static getAvailableTransports(): AuthenticatorTransport[] {
    const transports: AuthenticatorTransport[] = ['platform' as AuthenticatorTransport];
  ...
  private parseAttestationObject(buffer: ArrayBuffer) {
    ...
    return {
      ...
      authData: {
        ...
        credentialIdLength: new DataView(buffer.slice(53, 55)).getUint16(0, false),
  ...
  private async verifySignature(
    ...
    try {
      ...
      signedData.set(new Uint8Array(clientDataJSON), authenticatorData.byteLength);
  ...
  private getDeviceType(
    ...
    if (!transports) return 'single_device';
    return transports.includes('usb') || transports.includes('ble') || transports.includes('nfc')
      ? 'cross_platform'
      : 'single_device';
  }

  private getCredentialName(transports?: AuthenticatorTransport[], aaguid?: string): string {
