...
export class BiometricError extends Error {
  ...
  private getDefaultUserMessage(): string {
    const messages: Record<BiometricErrorCode, string> = {
      ...
      [BiometricErrorCode.NOT_AVAILABLE]:
        "Biometric authentication is currently unavailable. Please try again.",
      [BiometricErrorCode.HARDWARE_UNAVAILABLE]:
        "Your biometric hardware is not available. Please enable it in settings.",
      [BiometricErrorCode.TIMEOUT]:
        "Authentication request timed out. Please try again.",
      [BiometricErrorCode.NOT_ALLOWED]:
        "Biometric authentication was not allowed. Please check your permissions.",
      ...
      [BiometricErrorCode.INVALID_STATE]:
        "An invalid state was encountered. Please try again.",
      [BiometricErrorCode.CONSTRAINT_ERROR]:
        "Biometric constraint error. Please try again.",
      [BiometricErrorCode.DATA_ERROR]:
        "Data error occurred during biometric operation.",
      [BiometricErrorCode.OPERATION_ERROR]:
        "Operation error occurred. Please try again.",
      [BiometricErrorCode.INVALID_CREDENTIAL]:
        "Invalid biometric credential. Please register again.",
      [BiometricErrorCode.CREDENTIAL_MISMATCH]:
        "Biometric credential does not match. Please try again.",
      [BiometricErrorCode.VERIFICATION_FAILED]:
        "Biometric verification failed. Please try again.",
      [BiometricErrorCode.ATTESTATION_FAILED]:
        "Credential attestation failed. This credential may not be trustworthy.",
      [BiometricErrorCode.SESSION_EXPIRED]:
        "Your session has expired. Please authenticate again.",
      [BiometricErrorCode.SESSION_INVALID]:
        "Your session is invalid. Please authenticate again.",
      [BiometricErrorCode.INSUFFICIENT_PERMISSIONS]:
        "You do not have permission to perform this action.",
      [BiometricErrorCode.STORAGE_ERROR]:
        "Failed to store credential. Please try again.",
      [BiometricErrorCode.ENCRYPTION_ERROR]:
        "Encryption error occurred. Please try again.",
      [BiometricErrorCode.DECRYPTION_ERROR]:
        "Failed to decrypt credential. Please try again.",
      [BiometricErrorCode.RATE_LIMITED]:
        "Too many attempts. Please wait before trying again.",
      [BiometricErrorCode.TOO_MANY_ATTEMPTS]:
        "Too many failed attempts. Please use your PIN to authenticate.",
      [BiometricErrorCode.SERVER_ERROR]:
        "Server error occurred. Please try again later.",
      [BiometricErrorCode.DATABASE_ERROR]:
        "Database error occurred. Please try again later.",
      [BiometricErrorCode.CONFIGURATION_ERROR]:
        "Configuration error occurred. Please contact support.",
  ...
  suggestRecovery(): string {
    const suggestions: Record<BiometricErrorCode, string> = {
      ...
      [BiometricErrorCode.NOT_AVAILABLE]:
        "Try again or use PIN authentication.",
      [BiometricErrorCode.HARDWARE_UNAVAILABLE]:
        "Enable biometric hardware in device settings.",
      [BiometricErrorCode.TIMEOUT]: "Try the biometric scan again.",
      [BiometricErrorCode.NOT_ALLOWED]:
        "Grant biometric permissions in settings.",
      [BiometricErrorCode.NETWORK_ERROR]:
      ...
      [BiometricErrorCode.CREDENTIAL_MISMATCH]:
        "Try biometric authentication again.",
      [BiometricErrorCode.VERIFICATION_FAILED]:
        "Try biometric authentication again or use PIN.",
      [BiometricErrorCode.ATTESTATION_FAILED]:
        "Use a different biometric method or PIN.",
...
export function createBiometricError(
  ...
  return new BiometricError(defaultCode, String(error), undefined, context);
}

/**
 * Error logger for auditing
 */
export type BiometricErrorLog = {
  timestamp: Date;
  code: BiometricErrorCode;
  message: string;
  userId?: string;
