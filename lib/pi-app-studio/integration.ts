...
export const PI_APP_CONFIG: PiAppConfig = {
  ...
  version: '1.0.0',
  environment: (process.env.NODE_ENV === 'production' ? 'production' : 'sandbox') as PiAppConfig['environment'],
  
  sdk: {
    version: '2.0',
    scriptUrl: process.env.NEXT_PUBLIC_PI_SDK_URL || 'https://sdk.minepi.com/pi-sdk.js',
...
export async function initializePiSDK(): Promise<boolean> {
  // Pi SDK initialization is handled by PiProvider
  // This function is kept for backward compatibility but should not initialize SDK
  console.warn('[Pi App Studio] initializePiSDK called - SDK should be initialized by PiProvider');
  return true;
}
 ...
 */
export async function authenticatePiUser(scopes: string[] = ['username', 'payments']): Promise<PiUser | null> {
 ...
 */
export async function createPiPayment(config: PiPaymentConfig): Promise<PiPaymentResult> {
  if (typeof window === 'undefined' || !window.Pi) {
  ...
  if (config.amount < PI_APP_CONFIG.payments.minAmount) {
    return { success: false, error: `Minimum amount is ${PI_APP_CONFIG.payments.minAmount} Pi` };
  }
  
  if (config.amount > PI_APP_CONFIG.payments.maxAmount) {
    return { success: false, error: `Maximum amount is ${PI_APP_CONFIG.payments.maxAmount} Pi` };
  }
