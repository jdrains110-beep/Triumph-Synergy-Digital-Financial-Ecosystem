...
export const PI_APP_CONFIG: PiAppConfig = {
  ...
  payments: {
    minAmount: Number.parseFloat(process.env.PI_MIN_TRANSACTION || "10"),
    maxAmount: Number.parseFloat(process.env.PI_MAX_TRANSACTION || "100000"),
    internalMultiplier: Number.parseFloat(
      process.env.INTERNAL_PI_MULTIPLIER || "1.5"
    ),
    externalMultiplier: Number.parseFloat(
      process.env.EXTERNAL_PI_MULTIPLIER || "1.0"
    ),
    settlementNetwork: "stellar",
  },
...
export async function initializePiSDK(): Promise<boolean> {
  ...
  console.warn(
    "[Pi App Studio] initializePiSDK called - SDK should be initialized by PiProvider"
  );
  return true;
}
  ...
  try {
    const payment = await window.Pi.createPayment(
      ...
      },
      {
        ...
        onError: (error: Error, paymentInfo?: any) => {
