...
export class UnifiedPaymentRouter {
   ...
   * @private
   */
  private async routeApplePayment(paymentData: Record<string, unknown>): Promise<{
    success: boolean;
    processor: string;
    paymentId?: string;
    error?: string;
  ...
  private async routeLegacyPayment(
    ...
    console.warn(`Legacy payment method ${method} - consider migrating to Pi or Apple Pay`);
   ...
   */
  async getPaymentStats(periodDays: number = 30): Promise<{
  ...
  async validateConfiguration(): Promise<{
    ...
    const appleValidation = await this.appleProcessor.validateMerchantConfiguration();
    checks.applePay = {
    ...
    if (process.env.STELLAR_PAYMENT_ACCOUNT && process.env.STELLAR_PAYMENT_SECRET) {
      checks.stellar = {
        ready: true,
